...
Model the new rule on the existing `.github/sa-tools/*Rule.php`: global namespace, Symfony
license header, `implements Rule` with `getNodeType()`/`processNode()`, a stable
`identifier('symfony.<camelCase>')`, and the narrowest node type that sees the shape.

## The four traps

1. **`.gitignore` whitelist.** `.github/sa-tools/.gitignore` ignores `*` then re-includes
   each file by name. Add `!<Name>Rule.php`, or the file is silently un-tracked: it commits
   nothing, CI never loads it, and nothing errors.
2. **Registration is three files**, not one: the `require` in `phpstan-rules.php`, a
   `services:` entry tagged `phpstan.rules.rule` in `phpstan.dist.neon`, and the
   `.gitignore` whitelist above. Miss one and the rule simply never runs.
3. **`phpstan analyse` counts are nondeterministic** (result cache + parallel workers report
...
4. **The diff-lint only flags new errors.** `phpstan-diff.php` compares the PR against the
